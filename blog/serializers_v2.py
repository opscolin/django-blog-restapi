...
class TagSerializer(serializers.ModelSerializer):
    ...
    def get_count(self, obj):
        ...
        # tags = Tag.objects.annotate(counts=Count('post')).filter(counts__gt=0)
        # tags = Tag.objects.values('name').annotate(counts=Count('post'))
        if tags:
            # print('===>', tags)
...
class CoursePostsSerializer(serializers.ModelSerializer):
